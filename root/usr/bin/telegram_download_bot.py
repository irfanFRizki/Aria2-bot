...
Konfigurasi dibaca dari UCI OpenWrt (/etc/config/aria2bot)
"""
...
# ── UCI Config Reader ──────────────────────────────────────────────────────────
def _uci_get(key: str, default: str = "") -> str:
    """Baca konfigurasi dari UCI OpenWrt (/etc/config/aria2bot)."""
    try:
        result = subprocess.run(
            ["uci", "-q", "get", f"aria2bot.settings.{key}"],
...
def load_config():
    """Muat semua konfigurasi dari UCI. Fallback ke env var atau default."""
    token = (
        _uci_get("bot_token") or
        os.environ.get("TELEGRAM_BOT_TOKEN", "")
    )
    if not token or token == "xxxxxxxxxxxxxxxxxxxxxxxxx":
        print("⚠️  WARNING: Telegram Bot Token belum dikonfigurasi!")
        print("   Set via LuCI > Services > Aria2 Bot > Settings")
        print("   Atau: uci set aria2bot.settings.bot_token='TOKEN_ANDA'")

    return {
        "bot_token":        token,
        "aria2_rpc_url":    _uci_get("aria2_rpc_url",  "http://localhost:6800/jsonrpc"),
        "aria2_rpc_secret": _uci_get("aria2_rpc_secret", ""),
        "max_concurrent":   int(_uci_get("max_concurrent", "2")),
        "max_retry":        int(_uci_get("max_retry",      "3")),
    }
...
# Konfigurasi global (dibaca dari UCI)
TELEGRAM_BOT_TOKEN     = _cfg["bot_token"]
ARIA2_RPC_URL          = _cfg["aria2_rpc_url"]
ARIA2_RPC_SECRET       = _cfg["aria2_rpc_secret"]

# Queue Configuration
MAX_CONCURRENT_DOWNLOADS = _cfg["max_concurrent"]
MAX_RETRY_ATTEMPTS       = _cfg["max_retry"]
